"""Cascade-aggressive: even more emphasis on direct regression + sharper labels.

Builds on the Cascade-tuned win (0.760 m) with bigger steps in the same
direction:
  fine_sigma   0.3 → 0.25   (sharper still — Cell-size-ish label width)
  ce_fine_w    0.4 → 0.3
  ce_coarse_w  0.2 → 0.15
  mse_w        0.4 → 0.55   (now > half the loss is direct xy regression)

Will be combined with baseline Cascade + Cascade-tuned for a 15-model mega
ensemble (more diversity → better geom-median).
"""
import sys, time
...
def loc_stats(e):
    return {'median_err_m': float(np.median(e)), 'mean_err_m': float(e.mean()),
            'p90_err_m': float(np.percentile(e, 90)),
            'p10_err_m': float(np.percentile(e, 10)),
            'max_err_m': float(e.max())}
...
def main():
    ...
    print(f'config: mse_w={MSE_W} fine_sigma={FINE_SIGMA} '
          f'ce_fine_w={CE_FINE_W} ce_coarse_w={CE_COARSE_W}', flush=True)
    ...
    fine_xy, _, _ = data.build_heatmap_grid(cell_size=FINE_CELL)
    fine_mask = data.build_free_mask(fine_xy)
    coarse_xy, _, _ = data.build_heatmap_grid(cell_size=COARSE_CELL)
