"""Cascade-ultra: push mse_w even further (0.7) to test whether the trend
   from baseline (0.2) → tuned (0.4) → aggressive (0.55) → ultra (0.7)
   keeps improving median, or whether we've already saturated.

   fine_sigma   0.25 → 0.20   (label width ~ half a cell)
   ce_fine_w    0.3  → 0.2
   ce_coarse_w  0.15 → 0.1
   mse_w        0.55 → 0.7    (now 70% of loss is direct xy regression)

   Either it wins alone (-0.01 vs aggressive) or adds variant diversity to
   the mega-ensemble.
"""
import sys, time
...
FINE_SIGMA = 0.20; COARSE_SIGMA = 1.0
CE_FINE_W = 0.2; CE_COARSE_W = 0.1; MSE_W = 0.7
...
def main():
    ...
    print(f'device: {device}    config: mse={MSE_W} fσ={FINE_SIGMA}', flush=True)
    ...
    for seed in SEEDS:
        ...
        torch.save(m.state_dict(),
                    CKPT_DIR / f'A_random__CascadeUltra_s{seed}.pt')

    pts = np.stack([s['pred'] for s in seed_preds], axis=0)
    err_g = loc_err(geom_median(pts), y[te_A])
    print(f'\n=== Ultra 5-seed geom-median ===')
    print(f'  median={np.median(err_g):.3f}  mean={err_g.mean():.3f}  '
          f'p90={np.percentile(err_g, 90):.3f}', flush=True)
    np.savez(PRED_DIR / 'A_random__CascadeUltraEnsemble.npz',
              pred=geom_median(pts), y_true=y[te_A],
              err=err_g, test_idx=te_A)
