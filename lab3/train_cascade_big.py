"""Cascade-big: larger encoder (model_dim 192→256, num_sab 3→4) with strong
regularization, same Aggressive-style loss weights.

Tries to push individual median below 0.75 by giving the encoder more
capacity, with dropout 0.4 + weight decay 2e-3 to keep overfit in check
(small dataset, 1449 real samples). If this works individually, mixing it
into the ensemble may push honest 5-fold val below 0.70.
"""
import sys, time
...
CFG = dict(embed_dim=48, model_dim=256, num_heads=4, num_sab=4, dropout=0.4)
WEIGHT_DECAY = 2e-3                                       # ↑ from 1e-3 (more reg)
JITTER_DBM = 4.0; BATCH_SIZE = 64
...
def train_one(model, tl, vl, device, tag):
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if vloss < best - 1e-4:
            ...
            bad += 1
            if bad >= PATIENCE:
                print(f'  [{tag}] early stop @ ep {ep}, best {best:.4f}', flush=True)
                break
        if ep == 1 or ep % 40 == 0:
            print(f'  [{tag}] ep {ep:3d}  vloss {vloss:.4f}  best {best:.4f}', flush=True)
...
def main():
    ...
    print(f'device: {device}  model_dim=256 num_sab=4 dropout=0.4 wd=2e-3', flush=True)
    ...
    for seed in SEEDS:
        ...
        torch.save(m.state_dict(),
                    CKPT_DIR / f'A_random__CascadeBig_s{seed}.pt')
