"""Coarse-to-Fine Heatmap Cascade (architectural upgrade #B).

Two heads on the same Set Transformer encoder:
  coarse 10x8 cells of 1.6 m each  → catches gross "which region"
  fine   40x33 cells of 0.4 m each → refines within region

Inference uses cascade gating:
  p_fine_refined[c] ∝ softmax(fine)[c] · softmax(coarse)[parent(c)]

Targets the p90 outlier tail (currently 2.67 m with single-resolution heatmap)
without sacrificing median.  Combined with 5000 GP synth as before.
"""
import sys
import time
...
COARSE_CELL_SIZE = 1.6   # coarse grid: ~10x8 = 80 regions
FINE_CELL_SIZE   = 0.4   # fine grid:   40x33 = 1320 cells (same as plain heatmap)
FINE_SIGMA   = 0.4
COARSE_SIGMA = 1.0
...
def main():
    ...
    for seed in SEEDS:
        ...
        torch.save(model.state_dict(),
                    CKPT_DIR / f'A_random__Cascade_s{seed}.pt')
    ...
    print(f'  Heatmap + synth, best single:            0.838 m')
    print(f'  Heatmap + synth x 5-ens (geom-median):   0.836 m  (prior best)')
    print(f'  Cascade + synth best single:             {bs["median_err_m"]:.3f} m')
    print(f'  Cascade + synth x 5-ens (mean):          {em["median_err_m"]:.3f} m')
    print(f'  Cascade + synth x 5-ens (geom-median):   {eg["median_err_m"]:.3f} m')

    np.savez(PRED_DIR / 'A_random__CascadeEnsemble.npz',
              pred=ens_gm, y_true=y[te_A], err=loc_err(ens_gm, y[te_A]),
              test_idx=te_A)

    rows = [{'split': 'A_random', 'model': 'CascadeBestSingle',
              **bs, 'nll': None},
             {'split': 'A_random', 'model': 'CascadeEnsembleMean',
              **em, 'nll': None},
             {'split': 'A_random', 'model': 'CascadeEnsembleGeomMedian',
              **eg, 'nll': None}]
    csv = OUT_DIR / 'metrics.csv'
    if csv.exists():
        old = pd.read_csv(csv)
        old = old[~old['model'].isin(['CascadeBestSingle',
                                        'CascadeEnsembleMean',
                                        'CascadeEnsembleGeomMedian'])]
