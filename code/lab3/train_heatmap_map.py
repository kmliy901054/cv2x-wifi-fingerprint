"""Floor Plan CNN Cross-Attention + Heatmap (architectural upgrade #A).

The Heatmap model only uses psquare.pgm as a binary mask (free vs blocked).
This version adds a small CNN encoder that turns the floor plan into spatial
tokens, then cross-attends the pooled scan vector with those tokens so the
model finally has geometric awareness (corridors, walls, where APs typically
sit) instead of just blocked/free.

Combined with 5000 GP synth as before.  Target: beat 0.836 m prior best.
"""
import sys
import time
...
def main():
    ...
    print(f'[grid] {Gw} x {Gh} = {len(cell_xy)} cells, '
          f'{int(free_mask.sum())}/{len(cell_xy)} free', flush=True)
    ...
    for seed in SEEDS:
        ...
        model = models.SetTransformerHeatmapMap(
        ...
        if seed == SEEDS[0]:
            n_params = sum(p.numel() for p in model.parameters()
                            if p.requires_grad)
            n_map = sum(p.numel() for p in model.map_encoder.parameters()
                         if p.requires_grad)
            n_xattn = sum(p.numel() for p in model.cross_attn.parameters()
                           if p.requires_grad)
            print(f'[model] total: {n_params:,} ({n_params/1000:.1f}k)  '
                  f'map_enc: {n_map/1000:.1f}k  '
                  f'xattn: {n_xattn/1000:.1f}k  '
                  f'map_tokens: {model.map_encoder.num_tokens}', flush=True)
        ...
        torch.save(model.state_dict(),
                    CKPT_DIR / f'A_random__HeatmapMap_s{seed}.pt')
    ...
    print(f'  median={em["median_err_m"]:.3f}  '
          f'mean={em["mean_err_m"]:.3f}  '
          f'p90={em["p90_err_m"]:.3f}', flush=True)
    print(f'\n========== 5-seed ENSEMBLE (geometric median) ==========',
          flush=True)
    print(f'  median={eg["median_err_m"]:.3f}  '
          f'mean={eg["mean_err_m"]:.3f}  '
          f'p90={eg["p90_err_m"]:.3f}', flush=True)

    print('\n========== Updated Split A ladder ==========', flush=True)
    print(f'  Big + synth x 5-ens (MDN):               0.889 m')
    print(f'  Heatmap + synth x 5-ens (geom-median):   0.836 m  (prior best)')
    print(f'  Heatmap+Map best single:                 {bs["median_err_m"]:.3f} m')
    print(f'  Heatmap+Map x 5-ens (mean):              {em["median_err_m"]:.3f} m')
    print(f'  Heatmap+Map x 5-ens (geom-median):       {eg["median_err_m"]:.3f} m')

    np.savez(PRED_DIR / 'A_random__HeatmapMapEnsemble.npz',
              pred=ens_gm, y_true=y[te_A], err=loc_err(ens_gm, y[te_A]),
              test_idx=te_A)

    rows = [{'split': 'A_random', 'model': 'HeatmapMapBestSingle',
              **bs, 'nll': None},
             {'split': 'A_random', 'model': 'HeatmapMapEnsembleMean',
              **em, 'nll': None},
             {'split': 'A_random', 'model': 'HeatmapMapEnsembleGeomMedian',
              **eg, 'nll': None}]
    csv = OUT_DIR / 'metrics.csv'
    if csv.exists():
        old = pd.read_csv(csv)
        old = old[~old['model'].isin(['HeatmapMapBestSingle',
                                        'HeatmapMapEnsembleMean',
                                        'HeatmapMapEnsembleGeomMedian'])]
